...
def fetch_monobank_transactions(
    account_id, api_key, from_time, to_time, max_retries=5
):
    ...
    while retries <= max_retries:
        ...
        if response.status_code == 200:
            ...
            print(
                f"⚠️ Ліміт запитів Mono API перевищено. Очікуємо {wait_time} секунд..."
            )
            ...
            raise Exception(
                f"❌ Помилка API Mono: {response.status_code} - {response.text}"
            )
...
def get_monobank_accounts(api_key):
    ...
    print(
        f"❌ Помилка отримання account-info: {response.status_code} - {response.text}"
    )
    return "unknown", {}
    ...
    for item in mono_entries:
        ...
        for account_id, account_info in accounts.items():
            ...
            while chunk_start < to_dt:
                ...
                print(
                    f"🔄 Транзакції з {chunk_start.date()} по {chunk_end.date()}"
                )

                try:
                    txs = fetch_monobank_transactions(
                        account_id, api_key, from_time, to_time
                    )
            ...
            for tx in all_transactions:
                ...
                operation_amount_value = (
                    abs(tx.get("operationAmount", 0)) / 100
                )
                ...
                new_row[7] = CURRENCY_CODES.get(
                    operation_currency, operation_currency
                )
                new_row[8] = abs(
                    format_amount(tx.get("commissionRate", 0)) / 100
                )  # комісія
                new_row[9] = balance
                new_row[10] = tx.get("comment", "")
                new_row[11] = tx.get("counterName", "")
                new_row[12] = (
                    tx.get("counterEdrpou", 0)
                    if tx.get("counterEdrpou")
                    else ""
                )
