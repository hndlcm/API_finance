...
# PayPal API URL
BASE_URL = (
    "https://api-m.sandbox.paypal.com"  # змінити на live для реального акаунту
)
...
def get_access_token():
    ...
    response = requests.post(
        url, headers=headers, data=data, auth=(CLIENT_ID, SECRET)
    )
...
if __name__ == "__main__":
    ...
    resp = requests.get(
        f"{BASE_URL}/v1/reporting/transactions", headers=headers
    )
    ...
    for txn in transactions:
        ...
        print(
            f"{info['transaction_id']} - {info['transaction_amount']['value']} {info['transaction_amount']['currency_code']}"
        )
